...
func (t Training) distance() float64 {
	...
	result := float64(t.Action) * t.LenStep / float64(MInKm)
	return result
}
...
func (t Training) meanSpeed() float64 {
	...
	result := t.distance() / float64(durationInHours)
	return result
}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	firstMultiplier := float64(CaloriesMeanSpeedMultiplier)*r.meanSpeed() + float64(CaloriesMeanSpeedShift)
	secondMultiplier := r.Weight / float64(MInKm) * float64(r.Duration.Hours()) * float64(MinInHours)
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	firstMultiplier := float64(CaloriesWeightMultiplier) * w.Height
	speedMeterSec := w.meanSpeed() / 3.6
	secondMultiplier := math.Pow(speedMeterSec, 2) / w.Height
	thirdMultiplier := float64(CaloriesSpeedHeightMultiplier) * w.Weight
	fourthMultiplier := float64(w.Duration.Hours()) * float64(MinInHours)
	result := (firstMultiplier + secondMultiplier*thirdMultiplier) * fourthMultiplier
	return result
}
...
func (s Swimming) meanSpeed() float64 {
	...
	firstMultiplier := float64(s.LengthPool) * float64(s.CountPool)
	secondMultiplier := firstMultiplier / float64(MInKm)
	result := secondMultiplier / float64(s.Duration.Hours())
	return result
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	firstMultiplier := s.meanSpeed() + float64(SwimmingCaloriesMeanSpeedShift)
	secondMultiplier := float64(SwimmingCaloriesWeightMultiplier) * s.Weight * float64(s.Duration.Hours())
	result := firstMultiplier * secondMultiplier
	return result
}
